...
class _OthersPageState extends State<OthersPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: const OthersHome(),
    );
  }
}

class OthersHome extends StatelessWidget {
  const OthersHome({Key? key}) : super(key: key);
...
class OthersList extends StatefulWidget {
  OthersList({Key? key}) : super(key: key);
  @override
...
class _OthersListState extends State<OthersList> {
  ...
  void getdata() {
    Future<Uint8List> back = NetUtils.getJsonBytes(
        'http://1.117.239.54:8080/task?operation=getByTag&index='+Account.account+'&key=4');
    back.then((value) {
